...
export default {
    ...
    async loadCardtocards(context, payload) {
        ...
        try {
            console.log(payload)
            const {data:responseData} = await finAgent.get(`/front/card_to_card_raws?page=${payload.page}&per_page=${payload.itemsPerPage}&q[transaction_date_matches]=${payload.transactionDate}&q[amount_matches]=${payload.amount}&q[peygiri_number_matches]=${payload.peygiriNumber}&q[from_card_start]=${payload.fromCard}&q[to_card_start]=${payload.toCard}&q[serial_number_matches]=${payload.serialNumber}&q[is_used_eq]=${payload.isUsed}`)
